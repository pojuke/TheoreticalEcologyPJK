...
Here, ***N*** is a vector of population densities with elements $N_i$ (length n), $D(N)$ is a diagonal matrix with Ni on the main diagonal (dimension n \* n), r is a vector of species’ intrinsic growth rates $r_i$, and A is a matrix of interaction coefficients $\alpha_{ij}$ (dimension n \* n). With this notation, we can efficiently code the GLV with the following R script.
